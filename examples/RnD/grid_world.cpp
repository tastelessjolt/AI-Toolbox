...
using CoordType = std::array<int, 2>;
enum {
    AGENT_X = 0,
    AGENT_Y = 1,
};

CoordType terminalState = {5, 5};
...
double getTransitionProbability( const CoordType & c1, size_t action, const CoordType & c2 ) {
    ...
    int tigerMovementX = wrapDiff( c1[AGENT_X], c2[AGENT_X] );
    int tigerMovementY = wrapDiff( c1[AGENT_Y], c2[AGENT_Y] );
    ...
    int diffX = wrapDiff( c1[AGENT_X], terminalState[AGENT_X] );
    int diffY = wrapDiff( c1[AGENT_Y], terminalState[AGENT_Y] );
    ...
    double p = 3.0/4.0;
    if ( action == STAND && !( tigerMovementX || tigerMovementY ) ) return p;
    if ( action == UP    && tigerMovementY == 1  ) return p;
    if ( action == DOWN  && tigerMovementY == -1 ) return p;
    if ( action == LEFT  && tigerMovementX == -1 ) return p;
    if ( action == RIGHT && tigerMovementX == 1  ) return p;

    // Else the probability of this transition is (1 - p)/4, still random but without
    // a possible antelope action.
    return (1 - p)/(4.0);
}

double getReward( const CoordType & c ) {
    if ( c[AGENT_X] == terminalState[AGENT_X] && c[AGENT_Y] == terminalState[AGENT_Y] ) return 10.0;
    return 0.0;
}
...
class GridWorld {
    public:
        size_t getS() const { return SQUARE_SIZE * SQUARE_SIZE; }
        ...
        // This function is needed to keep template code in the library
...
void printState(const CoordType & c) {
    for ( int y = SQUARE_SIZE - 1; y >= 0; --y ) {
        for ( int x = 0; x < SQUARE_SIZE; ++x ) {
            if (x == c[AGENT_X] && y == c[AGENT_Y]) std::cout << "@ ";
            else if (x == terminalState[AGENT_X] && y == terminalState[AGENT_X]) std::cout << "G ";
            else std::cout << ". ";
        }
        std::cout << std::endl;
    }
